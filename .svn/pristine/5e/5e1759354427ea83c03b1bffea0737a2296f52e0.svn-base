...
use think\Db;
class Recruitcollection extends Common
{
    //显示浏览记录页面和查询记录
    public function index(){
        ...
        if($val){
         ...
         $adminList=Db::table('dzjn_recruit_collection')->alias('a')->join('dzjn_user_login l','a.c_lid = l.l_id')
             ->join('dzjn_recruit d','a.c_rid = d.r_id')
             ->join('dzjn_scmember_company f','d.r_companyid = f.c_id')
             ->where($map)->order("cc_time desc")->paginate(config('pageSize'));
        $adminList->appends($url);
        // 模板变量赋值
        $page = $adminList->render();
        $this->assign('page', $page);
        $this->assign('admin_list',$adminList);
        return $this->fetch('recruitcollection');
           
    }
        //根据判断条件以及获取到的手机号分别进行三个表查询并赋值，不行再新建三个详细页面
