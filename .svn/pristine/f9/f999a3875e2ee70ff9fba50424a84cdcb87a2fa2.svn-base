...
use think\Db;
use app\admin\model\Recruitbrowse as MemberModel;
class Recruitbrowse extends Common
{
    //显示浏览记录页面和查询记录
    public function index(){
        ...
        $adminList=$db->alias('a')->join('dzjn_user_login l','a.b_lid = l.l_id')
            ->join('dzjn_recruit c','a.b_rid = c.r_id')->join('dzjn_scmember_company f','c.r_companyid = f.c_id')
            ->where($map)->order("b_time desc")->paginate(config('pageSize'));
        ...
        return $this->fetch('recruitbrowse');
