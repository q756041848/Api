...
use think\Db;
class Recruitdelivery extends Common
{
    //显示浏览记录页面和查询记录
    public function index(){
        ...
        if($val){
         ...
         $adminList=Db::table('dzjn_recruit_delivery')->alias('a')->join('dzjn_user_login l','a.d_lid = l.l_id')
             ->join('dzjn_recruit g','a.d_rid = g.r_id')->join('dzjn_scmember_company f','g.r_companyid = f.c_id')
             ->where($map)->order("d_time desc")->paginate(config('pageSize'));
        $adminList->appends($url);
        // 模板变量赋值
        $page = $adminList->render();
        $this->assign('page', $page);
        $this->assign('admin_list',$adminList);
        return $this->fetch('recruitdelivery');
