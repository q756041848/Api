...
class Test extends Controller
{
    ...
    function test(){
        //$str = '草你大爷';
        //echo $this->pinyin1($str);
        $map['id'] = array('gt','3373');
        $res = db("data_region")->where($map)->select();
        foreach ($res as $k=>$v) {
            $tname = $v['name'];

            if($tname){
                try{
                    $d['letter'] =  $this->pinyin1( substr($tname,0,3));
                    $d['letters'] = $this->pinyin1($tname);
                    db("data_region")->where("id",$v['id'])->update($d);
                }catch (Exception $e){
